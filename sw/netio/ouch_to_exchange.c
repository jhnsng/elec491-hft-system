...
/* ============================================================
 * Lookup table: OUCH message type -> expected length
 * ============================================================ */
...
#define NUM_OUCH_TYPES 2
const ouch_type_info_t ouch_types[NUM_OUCH_TYPES] = {
	{ 'O', 45 },  // Enter Order
	{ 'X', 9  },  // Cancel Order
};
...
int main(void)
{
   ...
   //fifo = (unsigned int *)(lw_base + 0x08);   // FIFO data/meta
	...
	while (1) {
		...
		if (!first_sop_seen) {
        if (meta & META_SOP) {
            first_sop_seen = true;  // now start capturing
						msg_len = 0;
        } else {
            continue;               // ignore until SOP
        }
    }
		...
		//W	  meta, data, !!(meta & META_SOP), !!(meta & META_EOP));
...
//		}
		...
		if (meta & META_EOP) {
			...
			/* ----------------------------------------------------
			 * Lookup table validation for length prefix
			 * ---------------------------------------------------- */
			char msg_type = msg_buf[0];
			bool valid_type = false;
			for (int i = 0; i < NUM_OUCH_TYPES; i++) {
				if (ouch_types[i].type == msg_type &&
					ouch_types[i].length == msg_len) {
					valid_type = true;
					break;
				}
			}
			...
			if (!valid_type) {
				fprintf(stderr, "[WARN] Unknown OUCH type '%c' or length mismatch (%zu bytes)\n",
						msg_type, msg_len);
				msg_len = 0;
				continue;
			}
...
#if ENABLE_TCP
			/* ----------------------------------------------------
			 * Send 2-byte network-order length prefix
			 * ---------------------------------------------------- */
			uint16_t len_prefix = htons(msg_len);
			ssize_t sent = send(sock, &len_prefix, 2, 0);
			if (sent != 2) {
				perror("send length");
				break;
			}
			 ...
			 * Send payload
			 * ---------------------------------------------------- */
			sent = send(sock, msg_buf, msg_len, 0);
			if (sent < 0) {
				perror("send payload");
				break;
			}
